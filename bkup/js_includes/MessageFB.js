...
jqueryWidget: {
    _init: function () {
        ...
        this.style = (this.utils.getValueFromPreviousElement("failed")) ? "error" : "normal";
        this.htmlBad = this.options.htmlBad;
        this.htmlGood = this.options.htmlGood;
        ...
        //this.html = (this.style=="error") ? this.options.htmlWrong : this.options.htmlGood;
        this.element.addClass(this.cssPrefix + "messageFB");
        if (this.style == "error") {
            this.element.append(htmlCodeToDOM(this.htmlBad));
        }
        else {
            this.element.append(htmlCodeToDOM(this.htmlGood));
        }
        
        this.transfer_normal = dget(this.options, "normalTransfer", 500);
        assert((! this.transfer) || this.transfer == "click" || this.transfer == "click" || typeof(this.transfer) == "number",
               "Value of 'normalTransfer' option of MessageFB must either be the string 'click' or a number");

        this.transfer_error = dget(this.options, "errorTransfer", 500);
        assert((! this.transfer) || this.transfer == "click" || this.transfer == "click" || typeof(this.transfer) == "number",
               "Value of 'errorTransfer' option of MessageFB must either be the string 'keypress' or a number");
        
        this.transfer = (this.style=="error") ? this.transfer_error : this.transfer_normal;
...
properties: {
    obligatory: ["htmlGood","htmlBad"],
    countsForProgressBar: false,
    htmlDescription: function (opts) {
        return truncateHTML(htmlCodeToDOM(opts.htmlGood), 100);
    }
}
});
